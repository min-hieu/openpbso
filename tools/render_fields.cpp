...
void ListDirFiles(const char *dirname, std::vector<std::string> &names) {
    ...
    if ((dir = opendir(dirname)) != NULL) {
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (IsFile(f.c_str()) && (ent->d_name[0] != '.')) names.push_back(f);
        }
        closedir(dir);
    } else {
...
struct Plane {
    ...
    Plane() {
        ...
        lowCorner << PARAMS.center.at(0) - dims[0] / 2. * dx,
    ...
    void SetColor(const std::string &filename, igl::opengl::ViewerData &viewerData) {
        Load(filename);
    ...
    int AddToViewer(igl::opengl::glfw::Viewer &viewer) {
        ...
        for (int ii = 0; ii <= dims[0]; ++ii) {
            for (int jj = 0; jj <= dims[1]; ++jj) {
                V.row(ii * (dims[1] + 1) + jj) << lowCorner[0] + ii * dx,
                    lowCorner[1] + jj * dx,
                    lowCorner[2] + dims[2] / 2 * dx;
                if (ii != dims[0] && jj != dims[1]) {
                    F.row(2 * (ii * dims[1] + jj)) << ii * (dims[1] + 1) + jj,
                        (ii + 1) * (dims[1] + 1) + jj,
                        ii * (dims[1] + 1) + (jj + 1);
                    F.row(2 * (ii * dims[1] + jj) + 1) << (ii + 1) * (dims[1] + 1) + jj,
                        (ii + 1) * (dims[1] + 1) + jj + 1,
                        ii * (dims[1] + 1) + (jj + 1);
                }
            }
...
int main(int argc, char **argv) {
    ...
    for (int ii = 0; ii < modes->numModes(); ++ii) {
        ...
        for (int jj = 0; jj < modes->numDOF() / 3; ++jj) {
            U(jj, ii) = U_i.at(jj * 3 + 0) * VN(jj, 0) + U_i.at(jj * 3 + 1) * VN(jj, 1) + U_i.at(jj * 3 + 2) * VN(jj, 2);
        }
    ...
    std::sort(filenames.begin(), filenames.end(), [&](const std::string &a, const std::string &b) { return ParseID(a) < ParseID(b); });
    for (const auto &f : filenames) std::cout << "Read file: " << f << std::endl;
    Plane plane;
    ...
    for (auto b : drawModes) std::cout << b << " ";
    std::cout << std::endl;
    ...
    viewer.callback_pre_draw = [&](igl::opengl::glfw::Viewer &viewer) {
        ...
        for (int drawMode : drawModes) {
            const double omega = std::sqrt(modes->omegaSquared(drawMode) / PARAMS.density);
            Z += U.col(drawMode) * cos(omega * time);
        }
        igl::colormap(igl::COLOR_MAP_TYPE_PARULA, Z, PARAMS.obj_vmin, PARAMS.obj_vmax, C);
        viewer.data(obj_idx).set_colors(C);

        plane.SetColor(filenames.at(timeIdx % filenames.size()), viewer.data(plane_idx));
